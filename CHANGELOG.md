...
## [0.3.0] - 2026-06-30 - Skills, community files, lessons

### Added
- Five new skills under `skills/`: `cover-letter-writer`, `requirements-matrix`,
  `interview-prep`, `reference-manager`, `profile-sync` (joining the existing
  `cv-matcher`, `ats-optimizer`, `linkedin-profile-validator`).
- `CONTRIBUTING.md` and GitHub issue templates (`.github/ISSUE_TEMPLATE/`).
- `01_docs/LESSONS_LEARNED.md` (engineering lessons).

### Changed
- `.gitignore` comments translated to English; README cross-repo link uses a full
  GitHub URL. MIT `LICENSE` added (see v0.2.0 line).

### Verification
- PII sweep over publishable files clean; all SKILL.md have valid frontmatter.
